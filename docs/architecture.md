...
본 프로젝트는 **Gemini Engineering Principles**를 준수하여, 유지보수성과 확장성을 최우선으로 고려합니다.
...
- **Feature-based Structure**: 인증, 피드, 스케치 등 기능 단위로 폴더를 구성(`src/features/`)하여 응집도를 높입니다.
- **Shared Layer**: 전역적으로 사용되는 모달, 토스트, 유틸리티는 `src/shared/`에서 관리합니다.

### 2-2. State Management (Zustand)

- **Auth Store**: 사용자 세션 및 프로필 관리.
- **Feed Store**: 작품 목록, 탭 상태, 좋아요/삭제 동기화.
...
Vite 개발 서버와 Express API 서버의 충돌을 방지하기 위한 통합 구조를 사용합니다.
